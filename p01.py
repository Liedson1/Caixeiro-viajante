...
def read_tsp_file(file_path):
    ...
    if edge_weight_format == "LOWER_DIAG_ROW":
        ...
        if "DISPLAY_DATA_SECTION" in lines:
            display_data_section_start = lines.index("DISPLAY_DATA_SECTION\n") + 1
            display_data_section_end = lines.index("EOF\n", display_data_section_start)

            display_data = [list(map(int, line.split())) for line in lines[display_data_section_start:display_data_section_end] if line.strip()]
        else:
            display_data = None
